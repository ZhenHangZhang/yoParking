...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(changeHeaderImg:) name:NOTIFITION_HEADERIMG object:nil];
    UIImageView* bgview = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"leftbackiamge"]];
    bgview.frame = self.view.frame;
    [self.view addSubview:bgview];
    [self setTableView];
    [self setheadView];
}
-(void)changeHeaderImg:(NSNotification*)notifotion{
    [self setHeadeImg];
}
-(void)setTableView{
...
//    _headView.backgroundColor = [UIColor clearColor];
    ...
    [_headView addGestureRecognizer:tap];
     imgV = [[UIImageView alloc]initWithFrame:CGRectMake((self.view.frame.size.width - 100)/2 - 30, 220/2 - 30, 60, 60)];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
 ...
 AppDelegate *apdele = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    ...
    UIViewController *vc = nil;
    NSString *title = nil;
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:IS_LOGIN] integerValue] == 1) {
